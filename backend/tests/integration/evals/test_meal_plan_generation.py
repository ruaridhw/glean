...
from glean.meal_plan.schemas import MealPlanRecipe
from glean.meal_plan.service import MEAL_PLAN_SYSTEM_PROMPT

from .judges.rubrics import judge_meal_plan_recipe


def _invoke_meal_plan_generation(model: BaseChatModel, input_data: dict[str, Any], *, example_idx: int) -> str:
...
class TestMealPlanGenerationStructural:
    ...
    def test_all_examples_return_valid_json(
        ...
        for i, example in enumerate(meal_plan_generation_dataset):
            content = _invoke_meal_plan_generation(eval_model, example["input"], example_idx=i)
    ...
    def test_all_examples_conform_to_schema(
        ...
        for i, example in enumerate(meal_plan_generation_dataset):
            ...
            assert len(planned_recipes) > 0, f"Example {i}: returned empty list"
    ...
    def test_meal_plan_count_within_limit(
        ...
        for i, example in enumerate(meal_plan_generation_dataset):
            content = _invoke_meal_plan_generation(eval_model, example["input"], example_idx=i)
            planned_recipes = json.loads(content)
            limit = example["input"]["meals_per_week"]
            assert len(planned_recipes) <= limit, f"Example {i}: got {len(planned_recipes)} meals, limit is {limit}"
...
class TestMealPlanGenerationHeuristic:
    ...
    def test_missing_ingredients_not_in_pantry(
        ...
        for i, example in enumerate(meal_plan_generation_dataset):
            ...
            content = _invoke_meal_plan_generation(eval_model, example["input"], example_idx=i)
            planned_recipes = [MealPlanRecipe(**item) for item in json.loads(content)]
