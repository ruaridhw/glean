...
class TestOpenRouterConnectivity:
    def test_list_models_returns_results(self, openrouter_api_key: str, llm_model: str) -> None:
        ...
        assert llm_model in model_ids, f"{llm_model!r} not found in catalogue"

    def test_default_model_exists_in_catalogue(self, openrouter_api_key: str, llm_model: str) -> None:
        """The default model passes validate_model (raises ValueError if unknown)."""
        validate_model(llm_model, api_key=openrouter_api_key)


class TestOpenRouterChat:
    def test_simple_completion(self, openrouter_api_key: str, llm_model: str) -> None:
        """End-to-end: chat completion returns a response containing 'pong'."""
        model = create_chat_model(llm_model, api_key=openrouter_api_key)
